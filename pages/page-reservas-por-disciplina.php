...
-->
	...
	<div class="d-flex flex-column gap-2">
		...
		<div class="d-flex gap-1 justify-content-between align-items-end">
			<span><?= ($reservations_by_subject ? count($reservations_by_subject) : '0') ?> reserva(s)
				encontrada(s)</span>
					...
					foreach ($reservations_by_subject as $reservation):
						...
						<tr>
							...
							<td><?= implode(', ', array_map(function ($weekday) {
								return parse_weekday_to_name($weekday);
							}, (array) $reservation['data']['weekdays'])) ?>
							</td>
							<td>
						...
						<tbody>
							...
							if (!empty($times)):
								foreach ($times as $time):
									$limit_moves = $class_subject['data']['number_vacancies_offered'] ?? 0;
									...
									<tr>
										...
										<td scope="col">
											<a href="/assistente-de-reservas-de-salas/?subject=<?= $class_subject['id'] ?>&limit_moves=<?= $limit_moves ?>&start_time=<?= $time['start'] ?>&end_time=<?= $time['end'] ?>&weekdays=<?= $weekday ?>&frequency=weekly"
												class="btn btn-outline-primary" target="_blank"
												title="Reservar para <?= $time_display ?>">
