...
if (
    ...
    foreach ($old_reservations as $old_reservation) {
        ...
        if( $old_reservation['frequencia'] == 0 ) continue;
...
function get_old_class_subject_by_id($id, $class_subjects) {
    ...
    foreach( $class_subjects as $class_subject ) {
        if( $class_subject['id'] === $id ) return $class_subject;
    }
...
--> 
        ...
        <form class="my-3" action="/levantamento-reservas" method="POST">
            ...
            <div class="form-floating mb-3">
                <textarea class="form-control" placeholder="Insira o texto JSON aqui" id="floatingTextarea" name="old_class_subjects" rows="15" required></textarea>
                <label for="floatingTextarea">Diciplinas</label>
            </div>

            <div class="form-floating mb-3">
                <textarea class="form-control" placeholder="Insira o texto JSON aqui" id="floatingTextarea" name="old_reservations" rows="15" required></textarea>
        ...
        <h5><?= $total_count ?> reservas | <?= $above_counter ?> ineficientes(<?= round( $above_counter / 2, 2 ) ?>/semestre)</h5>
        <table class="table">
            <thead>
                <tr>
